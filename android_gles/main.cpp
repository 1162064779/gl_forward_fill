...
bool initEGL() {
  ...
  // 创建离屏Surface
  const EGLint surfaceAttribs[] = {EGL_WIDTH, windowWidth, EGL_HEIGHT,
                                   windowHeight, EGL_NONE};
...
std::string loadFileTest(const char *path) {
  ...
  LOGI("Shader %s  len=%zu  head(hex): %s  head(asc): %.16s", path, code.size(),
       hexBuf, ascBuf);

  return code;
}

inline void clearTextureRGBA8(GLuint tex, uint8_t r, uint8_t g, uint8_t b,
                              uint8_t a = 0) {
...
inline void clearTextureRGBA32UI(GLuint tex, uint32_t r, uint32_t g, uint32_t b,
                                 uint32_t a = 0) {
  static GLuint sFBO = 0;
  if (!sFBO)
    glGenFramebuffers(1, &sFBO);
...
int main() {
  ...
  LOGI("xptest: Max work group  count: %d x %d x %d", maxWGCount[0],
       maxWGCount[1], maxWGCount[2]);
  LOGI("xptest: Max work group  size : %d x %d x %d", maxWGSize[0],
       maxWGSize[1], maxWGSize[2]);
  LOGI("xptest: Max work group  invocations (x*y*z) : %d", maxWGInvoc);

  // 获取图像尺寸
  int imageW, imageH, channels;
  unsigned char *info_data =
      stbi_load("assets/avengers_sbs_d.png", &imageW, &imageH, &channels, 0);
  if (info_data) {
    windowWidth = imageW;
    windowHeight = imageH;
    ...
    LOGI("Failed to get image info, using default size: %dx%d", windowWidth,
         windowHeight);
  }
  ...
  GLuint imageTex =
      loadTextureFromPNG("assets/avengers_sbs_d.png", imageW, imageH);
  if (!imageTex) {
  ...
  // auto loadTime = duration_cast<milliseconds>(steady_clock::now() -
  // loadStart).count(); LOGI("xptest: Assets loaded in %lldms", loadTime);
  ...
  auto makeTargetTime =
      duration_cast<milliseconds>(steady_clock::now() - makeTargetStart)
          .count();
  LOGI("xptest: Target textures created in %lldms", makeTargetTime);

  // 编译着色器
  auto shaderStart = steady_clock::now();
  GLuint warpDProg = createComputeProgram("shaders/warp_depth.comp");
  GLuint warpCProg = createComputeProgram("shaders/warp_color.comp");
  loadFileTest("shaders/warp_depth.comp");
  loadFileTest("shaders/warp_color.comp");
  ...
  //     return -1;
  // }
  if (!warpDProg || !warpCProg || !tileProg) {
    LOGE("xptest: Failed to create compute programs");
    cleanupEGL();
    return -1;
  }
  ...
  LOGI("xptest:   orgWidth=%d, orgHeight=%d, eyeSign=%d", tileU.orgWidth,
       tileU.orgHeight, tileU.eyeSign);

  auto shaderTime =
      duration_cast<milliseconds>(steady_clock::now() - shaderStart).count();
  LOGI("xptest: Shaders compiled and uniforms cached in %lldms", shaderTime);

  // ---------------------------------------------------------------------------
  //  扭曲处理：Pass-1 统计最大深度（warpDProg）→ Pass-2
  //  根据深度写颜色/索引（warpCProg）
  // ---------------------------------------------------------------------------
  auto warpEye = [&](GLuint dstC, GLuint dstD, GLuint dstI, float xScale,
                     float yScale) {
    /* ---------- 预计算位移系数 ---------- */
    ...
    if (gx > (GLuint)maxWGCount[0] || gy > (GLuint)maxWGCount[1]) {
      LOGE(
          "xptest: Work group count exceeds hardware limits: %u x %u > %d x %d",
    ...
    auto setCommonUniforms = [&](GLuint prog) {
      glUniform1i(glGetUniformLocation(prog, "srcColor"), 0);
      glUniform1i(glGetUniformLocation(prog, "orgWidth"), imageW);
      glUniform1i(glGetUniformLocation(prog, "orgHeight"), imageH);
      glUniform1i(glGetUniformLocation(prog, "padSizeX"), padSizeX);
      glUniform1i(glGetUniformLocation(prog, "padSizeY"), padSizeY);
      glUniform1i(glGetUniformLocation(prog, "paddedWidth"), paddedW);
      glUniform1i(glGetUniformLocation(prog, "paddedHeight"), paddedH);
      glUniform1f(glGetUniformLocation(prog, "shiftScaleX"), shiftScaleX);
      glUniform1f(glGetUniformLocation(prog, "shiftBiasX"), shiftBiasX);
      glUniform1f(glGetUniformLocation(prog, "shiftScaleY"), shiftScaleY);
      glUniform1f(glGetUniformLocation(prog, "shiftBiasY"), shiftBiasY);
    };

    /* =============== Pass-1 : 最大深度 ================= */
    glUseProgram(warpDProg);

    // 输入纹理
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, imageTex);

    // 只用深度 image（binding = 3，R32UI）
    glBindImageTexture(3, dstD, 0, GL_FALSE, 0, GL_READ_WRITE, GL_R32UI);

    setCommonUniforms(warpDProg);

    glDispatchCompute(gx, gy, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT); // 等待深度写完

    /* =============== Pass-2 : 写颜色 / 索引 ============== */
    glUseProgram(warpCProg);
    ...
    glBindImageTexture(3, dstD, 0, GL_FALSE, 0, GL_READ_ONLY, GL_R32UI);
    glBindImageTexture(4, dstI, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32UI);

    setCommonUniforms(warpCProg);

    glDispatchCompute(gx, gy, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT); // 后续若要采样 dstC
  };
  ...
  auto fillEye = [&](GLuint color, GLuint index, int eyeSign) {
    ...
    if ((GLuint)numTile > (GLuint)maxWGCount[0] || TILE_W > maxWGSize[0]) {
      LOGE("xptest: Fill dispatch exceeds hardware limits: numTile=%d > %d or "
           "%d > %d",
  ...
  //     // 左眼
  //     glBindImageTexture(0, leftColor , 0, GL_FALSE, 0, GL_WRITE_ONLY,
  //     GL_RGBA8   ); glBindImageTexture(1, leftDepth , 0, GL_FALSE, 0,
  //     GL_WRITE_ONLY, GL_R32UI   ); glBindImageTexture(2, leftIndex , 0,
  //     GL_FALSE, 0, GL_WRITE_ONLY, GL_R32UI   ); glBindImageTexture(3,
  //     leftEdge  , 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32UI);
  //     // 右眼
  //     glBindImageTexture(4, rightColor, 0, GL_FALSE, 0, GL_WRITE_ONLY,
  //     GL_RGBA8   ); glBindImageTexture(5, rightDepth, 0, GL_FALSE, 0,
  //     GL_WRITE_ONLY, GL_R32UI   ); glBindImageTexture(6, rightIndex, 0,
  //     GL_FALSE, 0, GL_WRITE_ONLY, GL_R32UI   ); glBindImageTexture(7,
  //     rightEdge , 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32UI);

  //     /* ------- 计算 dispatch 尺寸（取所有贴图里最大的宽高） ------- */
  //     const int maxW = std::max(imageW, edgeW);      // edgeW 可能比 imageW
  //     大 const int groupX = (maxW  + 31) / 32; const int groupY = (imageH +
  //     31) / 32;
  ...
  //     auto us = duration_cast<microseconds>(steady_clock::now() -
  //     t0).count(); static bool first = true; if (first) {
  ...
  // 性能测试 - 完整流程(Warp+Fill)
  LOGI("xptest: Starting complete pipeline performance test (%d iterations)...",
       TEST_ITERATIONS);
  ...
  for (int i = 0; i < TEST_ITERATIONS; ++i) {
    ...
    // ---------- 4. 打印耗时 ----------
    auto iterTime =
        duration_cast<microseconds>(steady_clock::now() - iterStart).count();
    ...
    if (shouldPrint) {
      LOGI("xptest: Iteration %d/%d: %.2f ms", i + 1, TEST_ITERATIONS,
           iterTime / 1000.0);
    } else if (i == 10) {
      LOGI("xptest: ... (skipping middle iterations for brevity) ...");
    }
  }
  auto pipelineAvgTime =
      duration_cast<microseconds>(steady_clock::now() - pipelineTestStart)
          .count() /
      (double)TEST_ITERATIONS;
  LOGI("xptest: Complete pipeline average time: %.2f ms ",
       pipelineAvgTime / 1000.0);
  ...
  glDeleteProgram(warpDProg);
  glDeleteProgram(warpCProg);
